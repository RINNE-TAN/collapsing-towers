...
lemma preservation.maping.strengthened :
  ...
  case fvar 𝕊 x _ HBinds Hwbt Δ HEqΓ Hτv =>
    ...
    | lt =>
      ...
      apply typing.fvar
      . have HEq : x = x - ((τ𝕔, 𝟚) :: Φ).length + ((τ𝕒, 𝕊𝕒) :: Φ).length := by simp; omega
        rw [HEq]
        apply binds.extendr
        apply binds.shrinkr
        have HEq : x - ((τ𝕔, 𝟚) :: Φ).length + ((τ𝕔, 𝟚) :: Φ).length = x := by simp; omega
        rw [HEq]
        apply HBinds
      . apply Hwbt
    | eq =>
  ...
  case lam Hwbt Hclosed IH Δ HEqΓ Hτv =>
    ...
    rw [HEqΓ] at IH
    apply typing.lam
    . have HEq : (Δ ++ (τ𝕒, 𝕊𝕒) :: Φ).length = (Δ ++ (τ𝕔, 𝟚) :: Φ).length := by simp
...
theorem preservation.maping :
  ∀ Γ v e τ𝕒 τ𝕓 τ𝕔 𝕊 φ,
    typing ((τ𝕔, 𝟚) :: Γ) 𝟚 e τ𝕓 φ →
    typing ((τ𝕒, 𝕊) :: Γ) 𝟚 v τ𝕔 ⊥ →
    typing ((τ𝕒, 𝕊) :: Γ) 𝟚 (subst Γ.length v e) τ𝕓 φ :=
  by
