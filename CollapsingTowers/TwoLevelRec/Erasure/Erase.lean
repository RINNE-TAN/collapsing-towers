...
lemma identity.ty.erase_wbt_dyn : ∀ τ, wbt 𝟚 τ → ‖τ‖𝜏 = τ :=
  ...
  case arrow IH𝕒 IH𝕓 =>
    simp
    constructor; apply IH𝕒; apply Hwbt.right.left
    constructor; apply IH𝕓; apply Hwbt.right.right
    simp [Hwbt.left]
...
lemma identity.ty.erase_typing_dyn : ∀ Γ e τ φ, typing Γ 𝟚 e τ φ → ‖τ‖𝜏 = τ :=
  ...
  case fvar Hwbt HEq𝕊 =>
    ...
    apply Hwbt
  case lam Hτe Hwbt _ IHe HEq𝕊 =>
    rw [← HEq𝕊] at Hwbt Hτe
    constructor; apply identity.ty.erase_wbt_dyn; apply Hwbt
