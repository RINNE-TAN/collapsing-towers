...
mutual
  inductive typing : Store → TEnv → Stage → Expr → Ty → Meta → Effects → Prop where
    | fvar : ∀ σ Γ 𝕊 x τ,
      binds x (τ, 𝕊) Γ →
      wbt 𝕊 τ →
    ...
    | run : ∀ σ Γ e τ φ ω,
      ...
      closed e →
      typing σ Γ 𝟙 (.run e) (escape_ty τ) ⊥ (escape_effects ω)
    | unit : ∀ σ Γ 𝕊,
    ...
    | subsumption : ∀ σ Γ 𝕊 e τ φ ω₀ ω₁,
      ω₀ ≤ ω₁ →
      stage_effects 𝕊 (ω₁ \ ω₀) →
      typing σ Γ 𝕊 e τ φ ω₀ →
      typing σ Γ 𝕊 e τ φ ω₁

  inductive typing_reification : Store → TEnv → Expr → Ty → Meta → Effects → Prop
...
lemma typing.dynamic_impl_pure :
  ...
  revert HEq𝕊
  apply @typing.rec σ
    (fun Γ 𝕊 e τ φ ω (H : typing σ Γ 𝕊 e τ φ ω) => 𝟚 = 𝕊 → wbt 𝕊 τ ∧ φ = ⊥ ∧ stage_effects 𝕊 ω)
  ...
  case subsumption ω₀ ω₁ HLeω Hdiffω _ IH HEq𝕊 =>
    have ⟨Hwbt, Hφ, Hω₀⟩ := IH HEq𝕊
    constructor; apply Hwbt
    constructor; apply Hφ
    rw [← Set.union_diff_cancel HLeω]
    apply stage_effects.union _ _ _ Hω₀ Hdiffω
  case pure => simp
  case reify => simp
