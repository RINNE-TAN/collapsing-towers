...
lemma compatibility.fvar :
  ∀ Γ x τ,
    binds x (τ, 𝟙) Γ →
    log_equiv_typing Γ (.fvar x) (.fvar x) τ :=
  by
  intros Γ x τ Hbinds
...
lemma compatibility.lit :
  ∀ Γ n, log_equiv_typing Γ (.lit n) (.lit n) .nat :=
  by
  intros _ n
  constructor; constructor
  . constructor
...
lemma compatibility.lam :
  ∀ Γ e₀ e₁ τ𝕒 τ𝕓,
    ...
    closed_at (.lam e₁) Γ.length →
    log_equiv_typing ((τ𝕒, 𝟙) :: Γ) ({0 ↦ Γ.length} e₀) ({0 ↦ Γ.length} e₁) τ𝕓 →
    log_equiv_typing Γ (.lam e₀) (.lam e₁) (.arrow τ𝕒 τ𝕓 ∅) :=
  by
  ...
  simp only [multi_subst.lam, log_equiv_expr]
  ...
  rw [← comm.multi_subst_opening, comm.multi_subst_subst, ← multi_subst, HEq₁]
  apply Hsem; apply log_equiv_env.cons; apply Hsem_value; apply HsemΓ
...
lemma compatibility.app :
  ∀ Γ f₀ f₁ arg₀ arg₁ τ𝕒 τ𝕓,
    log_equiv_typing Γ f₀ f₁ (.arrow τ𝕒 τ𝕓 ∅) →
    log_equiv_typing Γ arg₀ arg₁ τ𝕒 →
    log_equiv_typing Γ (.app₁ f₀ arg₀) (.app₁ f₁ arg₁) τ𝕓 :=
  by
  ...
  . constructor; apply Hwf_f₁.right; apply Hwf_arg₁.right
  intros γ₀ γ₁ HsemΓ
  simp only [log_equiv_expr] at Hf Harg
  have ⟨Hmulti_wf₀, Hmulti_wf₁⟩ := log_equiv_env.multi_wf _ _ _ HsemΓ
  ...
  have ⟨e₀, e₁, HEq₀, HEq₁⟩ := log_equiv_value.arrow_ty_iff_lam lam₀ lam₁ _ _ Hsem_value_lam
  rw [HEq₀, HEq₁, log_equiv_value] at Hsem_value_lam
  have ⟨Hwf₀, Hwf₁, Hsem_value_lam⟩ := Hsem_value_lam
  apply log_equiv_expr.stepn; apply Hsem_value_lam; apply Hsem_value
  . simp
    -- left step
...
lemma compatibility.lets :
  ∀ Γ b₀ b₁ e₀ e₁ τ𝕒 τ𝕓,
    ...
    closed_at (.lets b₁ e₁) Γ.length →
    log_equiv_typing Γ b₀ b₁ τ𝕒 →
    log_equiv_typing ((τ𝕒, 𝟙) :: Γ) ({0 ↦ Γ.length} e₀) ({0 ↦ Γ.length} e₁) τ𝕓 →
    log_equiv_typing Γ (.lets b₀ e₀) (.lets b₁ e₁) τ𝕓 :=
  by
  ...
  have ⟨Hvalue₀, Hvalue₁⟩ := log_equiv_value.syntactic_value _ _ _ Hsem_value
  have ⟨Hwf₀, Hwf₁⟩ := log_equiv_value.wf _ _ _ Hsem_value
  apply log_equiv_expr.stepn; apply He
  apply log_equiv_env.cons; apply Hsem_value; apply HsemΓ
  . simp
    -- left step
    apply pure_stepn.trans
